...
def get_norm(norm, d_model):
    if norm == "BatchNorm":
        norm_attn = nn.Sequential(
            Transpose(1, 2),
            nn.BatchNorm1d(d_model),
            Transpose(1, 2),
        )
    elif norm == "LayerNorm":
        norm_attn = nn.LayerNorm(d_model)
    else:
        raise ValueError('norm must be "BatchNorm" or "LayerNorm"')
...
class TwoStageAttentionLayer(nn.Module):
    ...
    def __init__(self, seg_num, factor, d_model, n_heads, d_ff, dropout, norm):
        ...
        self.norm1 = get_norm(norm, d_model)
        self.norm2 = get_norm(norm, d_model)
        self.norm3 = get_norm(norm, d_model)
        self.norm4 = get_norm(norm, d_model)
